...
The function ~figedit-edit~ is used for editing existing figures. When called, the user is prompted to select a figure file. This file will then be opoened with ~figedit-edit-program~.
